...
if($_FILES['upload'])
{
    ...
    else if (!is_uploaded_file($_FILES['upload']["tmp_name"]))
    {
        ...
        move_uploaded_file($_FILES['upload']['tmp_name'], AS_ROOT."/uploads/content/".$name);
        $full_path = 'http://crm.pk-99.u0096264.plsk.regruhosting.ru/uploads/content/'.$name;
        $message = "Файл ".$_FILES['upload']['name']." загружен";
        $size=@getimagesize(AS_ROOT.'/uploads/content/'.$name);
        if($size[0]<50 OR $size[1]<50){
            unlink(AS_ROOT.'/uploads/content/'.$name);
